...
public class RatAgentBehaviour extends TickerBehaviour{
	...
	public RatAgentBehaviour(Agent agente, long tiempo) {
		super(agente, tiempo);
		this.punto= new Point(20,0,0);//new Point(Math.random()*200, Math.random()*200, 0.0);
	...
	private String generaCoordenadasHuir(InfoCollision info) {
		...
		case N: {if(parteConoColisionada.equals("izquierda")){
					...
					camino.getPunto().setX(camino.getPunto().getX()+0.1);
					camino.getPunto().setY(camino.getPunto().getY()-0.1);
				 }
	...
	private void nuevoMensaje(String destinatario, String contenido){
